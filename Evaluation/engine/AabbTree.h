...
template<typename Item> void AabbTree<Item>::insertItem(Item item) {
...
template<typename Item> bool AabbTree<Item>::update(Item item) {
	int index;
	if (_multithreadingEnabled) { requestWriteEnter(); }
...
template<typename Item> std::vector<Item> AabbTree<Item>::broadphase(const Vector2& point) const {
	...
	if (!isEmpty()) {
		...
		if (_multithreadingEnabled) { requestReadEnter(); }
		stack.push(_rootId);
		...
		if (_multithreadingEnabled) { requestReadExit(); }
	}
	return result;
}
...
template<typename Item> std::vector<Item> AabbTree<Item>::broadphase(const Aabb& aabb) const {
	...
	if (!isEmpty()) {
		...
		while (!stack.empty()) {
			...
			bool collisionTestResult = temp.aabb.intersects(aabb);
...
template<typename Item> void AabbTree<Item>::AabbTreeBuilder::initialize(AabbTree<Item>* tree, const std::vector<Item>& data, float leafMargin) {
	...
	tree->_multithreadingEnabled = Config.MultithreadingEnabled;
}
...
//// Fair
//
//template<typename Item> void AabbTree<Item>::requestWriteEnter() {
//	_requestQueueMtx.lock();
//	_accessMtx.lock();
//	_requestQueueMtx.unlock();
//}
//
//template<typename Item> void AabbTree<Item>::requestWriteExit() {
//	_accessMtx.unlock();
//}
//
//template<typename Item> void AabbTree<Item>::requestReadEnter() const {
//	_requestQueueMtx.lock();
//	_readersMtx.lock();
...
//}
//
//template<typename Item> void AabbTree<Item>::requestReadExit() const {
//	_readersMtx.lock();
//	--_currentReaders;
//	if (_currentReaders == 0) {
//		_accessMtx.unlock();
